...
namespace suplex {
    ...
    public:
                            ...
                            const std::shared_ptr<PrecomputeContext> context) override
        {
            ...
            shader->Bind();
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, context->Emu_BRDF_LUT.GetID());
            glBindFramebuffer(GL_FRAMEBUFFER, m_Framebuffer->GetID());
            glBindRenderbuffer(GL_RENDERBUFFER, m_Framebuffer->GetRenderbufferID());
            glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, 512, 512);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, context->Emu_BRDF_LUT.GetID(), 0);
            glViewport(0, 0, 512, 512);
